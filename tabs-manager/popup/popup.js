const tabs = await chrome.tabs.query({
    url: [
        "https://developer.chrome.com/docs/webstore/*", // bu sayfaları aç ama diğerlerini açma demek istiyoruz burada (örneğin https://developer.chrome.com/docs/extensions/*)
...
for(const tab of tabs) { // sekmeleri tek tek dolaşıyoruz
    const element = template.content.firstElementChild.firstElementChild.cloneNode(true); // <template> elementinin içindeki <li> elementini klonluyoruz
    // (true parametresi ile içindeki tüm alt elemanları da klonluyoruz) ve klonu alıyoruz (cloneNode() metodunun döndürdüğü değer klonlanan element)
    ...
    const pathname = new URL(tab.url).pathname.slice("/docs".length); // sekmelerin URL'lerini alıyoruz (örneğin "https://developer.chrome.com/docs/extensions/reference/tabs/")
    // ve gereksiz kısımları atıyoruz (örneğin "/docs")
